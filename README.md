...
| Test                | Description                                                                                  |
| ------------------- | -------------------------------------------------------------------------------------------- |
| JapanEsimPackage.js | Search for Japan → select Unlimited type → select 7-day plan → verify price matches Total |

## API test specs

| Spec                | Endpoint                | Description                                                            |
| ------------------- | ----------------------- | ---------------------------------------------------------------------- |
| getToken.test.ts    | `POST /v2/token`      | OAuth2 token — status 200, access_token present, token_type is Bearer |
| submitOrder.test.ts | `POST /v2/orders`     | Submit order for 6 eSIMs — status, message, iccid and qrcode per eSIM |
| getEsim.test.ts     | `GET /v2/sims/:iccid` | eSIM details for each iccid — status 200, iccid match, qrcode and lpa |
...
- SImple GitHub actions workflow added with the job for API tests
